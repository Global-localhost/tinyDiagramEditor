        ...
        */
        ...
        case STATE_SELECTING_MULTIPLE:
            ...
            */
            ...
            if(figuresToAdd.length >= 4){ //if we selected at least 2 figures then we can create a group
                selectedGroupId = STACK.groupCreate(figuresToAdd);
                state = STATE_GROUP_SELECTED;
            ...
            else if (figuresToAdd.length == 2){ // if we only select one figure, then it is not a group, it's a simple selection
...
function save()
    ...
    saveString(serializedDiagram,'Diagram.dgm');
    }

function newDiagramFile()
    {
    try
        {
        var tempDiagramData = '{"c":{"width":1920,"height":1080,"fillColor":"#ffffff","id":"canvasProps","oType":"CanvasProps"},"s":{"figures":[],"groups":[],"containers":[],"currentId":0,"idToIndex":[],"oType":"Stack"},"m":{"connectors":[],"connectionPoints":[],"connectionPointCurrentId":0,"glues":[]},"p":{"data":[]},"v":4}';
        var obj  = eval('(' + tempDiagramData + ')');
...
function copyObject()
    ...
    if(clipboardBuffer[0])
        ...
        switch(state)
            ...
            if(clipboardBuffer[0] == "figure")
                ...
                if(STACK.groupGetById(selectedGroupId))
                    {
                    //if this is true, then the group isn`t permanent
                    action('duplicate');    
                    }
                }
            break;
            ...
            if(clipboardBuffer[1] == selectedGroupId)
                ...
                if(clipboardBuffer[0] == "figure")
                    {
                    //if we have copied the figure, apply style to group
                    var copiedFigure = STACK.figureGetById(clipboardBuffer[1]);
                    var groupFigures = STACK.figureGetByGroupId(selectedGroupId);
                    for(var i=0; i<groupFigures.length; i++ )
                        {
        ...
        if (selectedConnectorId!=-1)
            {
            var con1 = CONNECTOR_MANAGER.connectorGetById(selectedConnectorId);

            var cps = CONNECTOR_MANAGER.connectionPointGetAllByParent(selectedConnectorId);
            var startX = cps[0].point.x;
            var startY = cps[0].point.y + 20;
            var endX   = cps[1].point.x;
            var endY   = cps[1].point.y + 20;
            ...
            connectorPickFirst(startX,startY, Connector.TYPE_STRAIGHT);
